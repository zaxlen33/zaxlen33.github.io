...
function openFestivalDetail(date) {
  ...
  renderFestivalUnderperformers(f, players);
}
...
function renderFestivalUnderperformers(f, players) {
  const card = document.getElementById('festival-underperformers-card');
  if (!card) return;
  ...
  if (!isUnlocked) {
    card.innerHTML = `
      <div class="perf-lock-zone">
        ...
        <div class="perf-form-group">
          <input type="password" class="perf-input" placeholder="${t('perf_enter_key')}" id="fest-perf-pass">
          <button class="btn perf-btn-unlock" id="fest-perf-btn">${t('perf_unlock_btn')}</button>
        </div>
        <div class="perf-error-msg" id="fest-perf-err">${t('perf_wrong_password')}</div>
      </div>
    `;

    const input = document.getElementById('fest-perf-pass');
    const btn = document.getElementById('fest-perf-btn');
    const err = document.getElementById('fest-perf-err');
    ...
    const underperformers = players.filter(p => p.score < FESTIVAL_MIN);
          ...
          <table>
            <thead>
              <tr>
                ...
                <th class="right">${t('score')}</th>
                <th class="center">${t('completed')}</th>
                ...
                return `
                  <tr class="perf-row-fail">
                    ...
                    <td style="font-weight:500;">
                      <a href="player.html?view=all&id=${encodeURIComponent(p.name)}" class="perf-link">${p.name}</a>
                    </td>
                    <td class="right mono" style="color:var(--accent-red); font-weight:700;">${fmtComma(p.score)}</td>
                    <td class="center">${p.completed}/${p.total}</td>
                    <td class="center"><span class="badge-not-met">❌ ${fmtComma(p.score)} / ${fmtComma(FESTIVAL_MIN)}</span></td>
                  </tr>
                `;
              }).join('')}
            </tbody>
    ...
    card.innerHTML = `
      <div class="perf-unlocked-header">
        ...
        <button class="perf-btn-lock" id="fest-perf-lock">${t('perf_lock_btn')}</button>
      </div>
    ...
    document.getElementById('fest-perf-lock').addEventListener('click', () => {
